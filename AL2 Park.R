...
for(kk in 1:10){
  ...
  Iselect <- ALC_two_level(x, fit.closed, 100, c(1,cost), list(park91alc, park91a), parallel=TRUE, ncore=10)
  ...
  while(park.cost[length(park.cost)] < 30){ # if total cost is less than the budget
    ...
    Iselect <- ALC_two_level(x, Iselect$fit, 100, c(1,cost), list(park91alc, park91a), parallel=TRUE, ncore=10)
