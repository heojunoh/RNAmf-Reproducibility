...
for(kk in 1:10){
  ...
  Iselect <- ALC_two_level(x, fit.closed, 100, c(1,cost), list(f1, f2), parallel=TRUE, ncore=10)
  ...
  while(perd.cost[length(perd.cost)] < 50){ # if total cost is less than the budget
    ...
    Iselect <- ALC_two_level(x, Iselect$fit, 100, c(1,cost), list(f1, f2), parallel=TRUE, ncore=10)
