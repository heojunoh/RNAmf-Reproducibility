...
for(kk in 1:10){
  ...
  if (alc.RNAmf$chosen$level == 1) {
    fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf$fit1$X)*attr(fit.RNAmf$fit1$X,"scaled:scale")+attr(fit.RNAmf$fit1$X,"scaled:center")), alc.RNAmf$chosen$Xnext),
                                      c(fit.RNAmf$fit1$y, apply(alc.RNAmf$chosen$Xnext,1,park91alc)),
    ...
    fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf$fit1$X)*attr(fit.RNAmf$fit1$X,"scaled:scale")+attr(fit.RNAmf$fit1$X,"scaled:center")), alc.RNAmf$chosen$Xnext),
                                      ...
                                      c(fit.RNAmf$fit2$y, apply(alc.RNAmf$chosen$Xnext,1,park91a)),
                                      kernel="sqex", constant=TRUE)
  }
  ...
  while(park.cost[length(park.cost)] < 30){ # if total cost is less than the budget
    ...
    if (alc.RNAmf$chosen$level == 1) {
      fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf.next$fit1$X)*attr(fit.RNAmf.next$fit1$X,"scaled:scale")+attr(fit.RNAmf.next$fit1$X,"scaled:center")), alc.RNAmf$chosen$Xnext),
                                        c(fit.RNAmf.next$fit1$y, apply(alc.RNAmf$chosen$Xnext,1,park91alc)),
      ...
      fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf.next$fit1$X)*attr(fit.RNAmf.next$fit1$X,"scaled:scale")+attr(fit.RNAmf.next$fit1$X,"scaled:center")), alc.RNAmf$chosen$Xnext),
                                        ...
                                        c(fit.RNAmf.next$fit2$y, apply(alc.RNAmf$chosen$Xnext,1,park91a)),
                                        kernel="sqex", constant=TRUE)
    }
    
    # save.image("/Park AL2 1,3.RData")
  }

  ### Save results ###
  costmatc2[[kk]] <- park.cost
  rmsematc2[[kk]] <- park.error
  crpsmatc2[[kk]] <- park.crps
  
  time.each2[kk] <- proc.time()[3]- time.start
  # save.image("Park ALC.RData")
}
costmatc2
rmsematc2
crpsmatc2
