...
for(kk in 1:10){
  ...
  ald.RNAmf <- ALD_RNAmf(Xcand = X.test, fit.RNAmf, cost = cost, optim = TRUE, parallel = parallel, ncore=ncore)
  ...
  if (ald.RNAmf$chosen$level == 1) {
    fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf$fit1$X)*attr(fit.RNAmf$fit1$X,"scaled:scale")+attr(fit.RNAmf$fit1$X,"scaled:center")), ald.RNAmf$chosen$Xnext),
                                      c(fit.RNAmf$fit1$y, blade1(ald.RNAmf$chosen$Xnext)),
    ...
    fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf$fit1$X)*attr(fit.RNAmf$fit1$X,"scaled:scale")+attr(fit.RNAmf$fit1$X,"scaled:center")), ald.RNAmf$chosen$Xnext),
                                      ...
                                      c(fit.RNAmf$fit2$y, blade2(ald.RNAmf$chosen$Xnext)),
                                      kernel="sqex", constant=TRUE)
  }
  ...
  while(blade.cost[length(blade.cost)] < 45.5){ # if total cost is less than the budget
    ...
    ald.RNAmf <- ALD_RNAmf(Xcand = X.test, fit.RNAmf.next, cost = cost,
    ...
    if (ald.RNAmf$chosen$level == 1) {
      fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf.next$fit1$X)*attr(fit.RNAmf.next$fit1$X,"scaled:scale")+attr(fit.RNAmf.next$fit1$X,"scaled:center")), ald.RNAmf$chosen$Xnext),
                                        c(fit.RNAmf.next$fit1$y, blade1(ald.RNAmf$chosen$Xnext)),
      ...
      fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf.next$fit1$X)*attr(fit.RNAmf.next$fit1$X,"scaled:scale")+attr(fit.RNAmf.next$fit1$X,"scaled:center")), ald.RNAmf$chosen$Xnext),
                                        ...
                                        c(fit.RNAmf.next$fit2$y, blade2(ald.RNAmf$chosen$Xnext)),
                                        kernel="sqex", constant=TRUE)
    }
    
    # save.image("Blade AL4.RData")
  }
  
  
  ### Save results ###
  costmatc4[[kk]] <- blade.cost
  rmsematc4[[kk]] <- blade.error
  crpsmatc4[[kk]] <- blade.crps
  
  time.each4[kk] <- proc.time()[3]- time.start
  # save.image("Blade ALD.RData")
}
costmatc4
rmsematc4
crpsmatc4
