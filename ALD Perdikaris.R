...
for(kk in 1:10){
  ...
  if (ald.RNAmf$chosen$level == 1) {
    fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf$fit1$X)*attr(fit.RNAmf$fit1$X,"scaled:scale")+attr(fit.RNAmf$fit1$X,"scaled:center")), ald.RNAmf$chosen$Xnext),
                                      c(fit.RNAmf$fit1$y, f1(ald.RNAmf$chosen$Xnext)),
    ...
    fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf$fit1$X)*attr(fit.RNAmf$fit1$X,"scaled:scale")+attr(fit.RNAmf$fit1$X,"scaled:center")), ald.RNAmf$chosen$Xnext),
                                      c(fit.RNAmf$fit1$y, f1(ald.RNAmf$chosen$Xnext)),
                                      rbind(matrix(t(t(fit.RNAmf$fit2$X)*attr(fit.RNAmf$fit2$X,"scaled:scale")+attr(fit.RNAmf$fit2$X,"scaled:center"))[,1]), ald.RNAmf$chosen$Xnext),
                                      c(fit.RNAmf$fit2$y, f2(ald.RNAmf$chosen$Xnext)),
                                      kernel="sqex", constant=TRUE)
  }
  ...
  while(perd.cost[length(perd.cost)] < 50){ # if total cost is less than the budget
    ...
    if (ald.RNAmf$chosen$level == 1) {
      fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf.next$fit1$X)*attr(fit.RNAmf.next$fit1$X,"scaled:scale")+attr(fit.RNAmf.next$fit1$X,"scaled:center")), ald.RNAmf$chosen$Xnext),
                                        c(fit.RNAmf.next$fit1$y, f1(ald.RNAmf$chosen$Xnext)),
      ...
      fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf.next$fit1$X)*attr(fit.RNAmf.next$fit1$X,"scaled:scale")+attr(fit.RNAmf.next$fit1$X,"scaled:center")), ald.RNAmf$chosen$Xnext),
                                        c(fit.RNAmf.next$fit1$y, f1(ald.RNAmf$chosen$Xnext)),
                                        rbind(matrix(t(t(fit.RNAmf.next$fit2$X)*attr(fit.RNAmf.next$fit2$X,"scaled:scale")+attr(fit.RNAmf.next$fit2$X,"scaled:center"))[,1]), ald.RNAmf$chosen$Xnext),
                                        c(fit.RNAmf.next$fit2$y, f2(ald.RNAmf$chosen$Xnext)),
                                        kernel="sqex", constant=TRUE)
    }

    # save.image("/Perd AL4 1,3.RData")
  }

  ### Save results ###
  costmatc4[[kk]] <- perd.cost
  rmsematc4[[kk]] <- perd.error
  crpsmatc4[[kk]] <- perd.crps

  time.each[kk] <- proc.time()[3]- time.start
  # save.image("Perd ALD.RData")
}
costmatc4
rmsematc4
crpsmatc4
