...
for(kk in 1:10){
  ...
  Iselect <- ALM_two_level(fit.RNAmf, cost, list(blade1, blade2), parallel=TRUE, ncore=10)
  ...
  while(blade.cost[length(blade.cost)] < 45.5){ # if total cost is less than the budget
    ...
    Iselect <- ALM_two_level(Iselect$fit, cost, list(blade1, blade2), parallel=TRUE, ncore=10)
