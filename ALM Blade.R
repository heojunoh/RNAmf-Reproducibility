...
for(kk in 1:10){
  ...
  alm.RNAmf <- ALM_RNAmf(Xcand = X.test, fit.RNAmf, cost = cost, optim = TRUE, parallel = parallel, ncore=ncore)
  
  if (alm.RNAmf$chosen$level == 1) {
    fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf$fit1$X)*attr(fit.RNAmf$fit1$X,"scaled:scale")+attr(fit.RNAmf$fit1$X,"scaled:center")), alm.RNAmf$chosen$Xnext),
                                      c(fit.RNAmf$fit1$y, blade1(alm.RNAmf$chosen$Xnext)),
    ...
    fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf$fit1$X)*attr(fit.RNAmf$fit1$X,"scaled:scale")+attr(fit.RNAmf$fit1$X,"scaled:center")), alm.RNAmf$chosen$Xnext),
                                      c(fit.RNAmf$fit1$y, blade1(alm.RNAmf$chosen$Xnext)),
                                      rbind(t(t(fit.RNAmf$fit2$X)*attr(fit.RNAmf$fit2$X,"scaled:scale")+attr(fit.RNAmf$fit2$X,"scaled:center"))[,-ncol(fit.RNAmf$fit2$X)], alm.RNAmf$chosen$Xnext),
                                      c(fit.RNAmf$fit2$y, blade2(alm.RNAmf$chosen$Xnext)),
                                      kernel="sqex", constant=TRUE)
  }
  ...
  while(blade.cost[length(blade.cost)] < 45.5){ # if total cost is less than the budget
    ...
    alm.RNAmf <- ALM_RNAmf(Xcand = X.test, fit.RNAmf.next, cost = cost,
                           optim = TRUE, parallel = parallel, ncore = ncore)
    
    if (alm.RNAmf$chosen$level == 1) {
      fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf.next$fit1$X)*attr(fit.RNAmf.next$fit1$X,"scaled:scale")+attr(fit.RNAmf.next$fit1$X,"scaled:center")), alm.RNAmf$chosen$Xnext),
                                        c(fit.RNAmf.next$fit1$y, blade1(alm.RNAmf$chosen$Xnext)),
      ...
      fit.RNAmf.next <- RNAmf_two_level(rbind(t(t(fit.RNAmf.next$fit1$X)*attr(fit.RNAmf.next$fit1$X,"scaled:scale")+attr(fit.RNAmf.next$fit1$X,"scaled:center")), alm.RNAmf$chosen$Xnext),
                                        c(fit.RNAmf.next$fit1$y, blade1(alm.RNAmf$chosen$Xnext)),
                                        rbind(t(t(fit.RNAmf.next$fit2$X)*attr(fit.RNAmf.next$fit2$X,"scaled:scale")+attr(fit.RNAmf.next$fit2$X,"scaled:center"))[,-ncol(fit.RNAmf$fit2$X)], alm.RNAmf$chosen$Xnext),
                                        c(fit.RNAmf.next$fit2$y, blade2(alm.RNAmf$chosen$Xnext)),
                                        kernel="sqex", constant=TRUE)
    }
    
    # save.image("Blade AL.RData")
  }
  
  
  ### Save results ###
  costmatc[[kk]] <- blade.cost
  rmsematc[[kk]] <- blade.error
  crpsmatc[[kk]] <- blade.crps
  
  time.each[kk] <- proc.time()[3]- time.start
  # save.image("Blade ALM.RData")
}
costmatc
rmsematc
crpsmatc
