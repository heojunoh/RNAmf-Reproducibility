...
for(kk in 1:10){
  ...
  Iselect <- ALMC_two_level(X.test, fit.RNAmf, 100, cost, list(park91alc, park91a), parallel=TRUE, ncore=10)
  ...
  while(blade.cost[length(blade.cost)] < 45.5){ # if total cost is less than the budget
    ...
    Iselect <- ALMC_two_level(X.test, Iselect$fit, 100, cost, list(park91alc, park91a), parallel=TRUE, ncore=10)
