...
if(ALC){
  ...
  pperdfinal <- ggplot(data.frame(RMSE=c(resultmatc[44,],resultmatc2[44,],resultmatc3[44,],resultmatc4[44,],resultmatco[44,],resultmatk[44,]), Strategy=Strategy), 
    ...
    theme(axis.title.x = element_text(margin = margin(t = 10), hjust=0.5),
          ...
          plot.margin = margin(t = 0, r = 30, b = -20, l = 10),
  ...
  pperdfinal <- ggplot(data.frame(RMSE=c(resultmatc[44,],resultmatc3[44,],resultmatc4[44,],resultmatco[44,],resultmatk[44,]), Strategy=Strategy), 
                       aes(x=Strategy, y=RMSE, fill=Strategy)) + 
    geom_boxplot(alpha=0.5) + theme_bw() + ggtitle("") +
    theme(axis.title.x = element_text(margin = margin(t = 10), hjust=0.5),
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5),
...
if(ALC){
  df.propperd <- data.frame(Prop=c(prop[,1], prop[,2], prop[,3], prop[,4], prop[,5], prop[,6]), 
                            Strategy=c(rep("ALM",10),rep("ALC",10),rep("ALMC",10),rep("ALD",10),rep("Cokriging-CV",10),rep("MR-SUR",10)))
  
  df.propperd$Strategy <- factor(df.propperd$Strategy , levels=c("ALD", "ALM", "ALC", "ALMC", "Cokriging-CV", "MR-SUR"))
}else{
  df.propperd <- data.frame(Prop=c(prop[,1], prop[,3], prop[,4], prop[,5], prop[,6]), 
                            Strategy=c(rep("ALM",10),rep("ALMC",10),rep("ALD",10),rep("Cokriging-CV",10),rep("MR-SUR",10)))
  
  df.propperd$Strategy <- factor(df.propperd$Strategy , levels=c("ALD", "ALM", "ALC", "ALMC", "Cokriging-CV", "MR-SUR"))
}

ppropperd <- ggplot(df.propperd, aes(x=Strategy, y=Prop, fill=Strategy)) + 
  geom_boxplot(alpha=0.5) + theme_bw() + ggtitle("") +
  theme(axis.title.x = element_text(margin = margin(t = 10), hjust=0.5, size=20),
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5),
        text=element_text(size=16), legend.position="none",
        panel.border = element_blank(), plot.margin = margin(t = 0, r = 10, b = -20, l = 30),
        plot.title = element_text(hjust = 0.5, size=20))+
  labs(x="", y = "Proportion of low-fidelity data")

plotalperd2 <- ggarrange(pperdfinal, ppropperd, legend= "none")
plotalperd2