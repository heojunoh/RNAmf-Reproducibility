...
if(ALC){
  ...
  pbladefinal <- ggplot(data.frame(RMSE=c(resultmatc[911,],resultmatc2[911,],resultmatc3[911,],resultmatc4[911,],resultmatco[911,],resultmatk[911,]), Strategy=Strategy), 
    ...
    theme(axis.title.x = element_text(margin = margin(t = 10), hjust=0.5),
          ...
          plot.title = element_text(hjust = 0.5, size=20)) + labs(x="", y = "Final RMSE")
}else{
  ...
  pbladefinal <- ggplot(data.frame(RMSE=c(resultmatc[911,],resultmatc3[911,],resultmatc4[911,],resultmatco[911,],resultmatk[911,]), Strategy=Strategy), 
                        aes(x=Strategy, y=RMSE, fill=Strategy)) + 
    geom_boxplot(alpha=0.5) + theme_bw() + ggtitle("") +
    theme(axis.title.x = element_text(margin = margin(t = 10), hjust=0.5),
          axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5),
...
for(j in 1:10){
  ...
  for(i in 1: length(costmatc[[j]])-1){
    if(isTRUE(abs(costmatc[[j]][i+1] - costmatc[[j]][i] - (2.25+6.85)) < 1e-3)){denom1 <- denom1 + 1}
  }
  if(ALC){
    for(i in 1: length(costmatc2[[j]])-1){
      if(isTRUE(abs(costmatc2[[j]][i+1] - costmatc2[[j]][i] - (2.25+6.85)) < 1e-3)){denom2 <- denom2 + 1}
    }
  }
  for(i in 1: length(costmatc3[[j]])-1){
    if(isTRUE(abs(costmatc3[[j]][i+1] - costmatc3[[j]][i] - (2.25+6.85)) < 1e-3)){denom3 <- denom3 + 1}
  }
  for(i in 1: length(costmatc4[[j]])-1){
    if(isTRUE(abs(costmatc4[[j]][i+1] - costmatc4[[j]][i] - (2.25+6.85)) < 1e-3)){denom4 <- denom4 + 1}
  }
  for(i in 1: length(costmatco[[j]])-1){
    if(isTRUE(abs(costmatco[[j]][i+1] - costmatco[[j]][i] - (2.25+6.85)) < 1e-3)){denomco <- denomco + 1}
  }
  for(i in 1: length(costmatk[[j]])-1){
    if(isTRUE(abs(costmatk[[j]][i+1] - costmatk[[j]][i] - 2.25) < 1e-3)){denomk <- denomk + 1}
  }
...
if(ALC){
  df.propblade <- data.frame(Prop=c(prop[,1], prop[,2], prop[,3], prop[,4], prop[,5], prop[,6]), 
                             Strategy=c(rep("ALM",10),rep("ALC",10),rep("ALMC",10),rep("ALD",10),rep("Cokriging-CV",10),rep("MR-SUR",10)))
  
  df.propblade$Strategy <- factor(df.propblade$Strategy , levels=c("ALD", "ALM", "ALC", "ALMC", "Cokriging-CV", "MR-SUR"))
}else{
  df.propblade <- data.frame(Prop=c(prop[,1], prop[,3], prop[,4], prop[,5], prop[,6]), 
                             Strategy=c(rep("ALM",10),rep("ALMC",10),rep("ALD",10),rep("Cokriging-CV",10),rep("MR-SUR",10)))
  
  df.propblade$Strategy <- factor(df.propblade$Strategy , levels=c("ALD", "ALM", "ALMC", "Cokriging-CV", "MR-SUR"))
}

ppropblade <- ggplot(df.propblade, aes(x=Strategy, y=Prop, fill=Strategy)) + 
  geom_boxplot(alpha=0.5) + theme_bw() + ggtitle("") +
...
plotalblade2 <- ggarrange(pbladefinal, ppropblade, legend = "none")
plotalblade2
