# CRPS
resultmatc <- resultmatc2 <- resultmatc3 <- resultmatc4 <- resultmatco <- resultmatk <- matrix(, nrow=31, ncol=10)
...
for(j in 1:10){
  ...
  resultmatc[,j] <- c(r1, crpsmatc[[j]][length(costmatc[[j]])])[1:31]
  if(ALC) resultmatc2[,j] <- c(r2, crpsmatc2[[j]][length(costmatc2[[j]])])[1:31]
  resultmatc3[,j] <- c(r3, crpsmatc3[[j]][length(costmatc3[[j]])])[1:31]
  resultmatc4[,j] <- c(r4, crpsmatc4[[j]][length(costmatc4[[j]])])[1:31]
  resultmatco[,j] <- c(rco, crpsmatco[[j]][length(costmatco[[j]])])[1:31]
  resultmatk[,j] <- c(rk, crpsmatk[[j]][length(costmatk[[j]])])[1:31]
}
...
if(ALC){
  parkalresult.crps <- data.frame(x=rep(100:130,6), 
                                  Strategy=factor(c(rep("ALM",31), rep("ALC",31), rep("ALMC",31), rep("ALD",31), rep("Cokriging-CV",31), rep("MR-SUR",31)), 
  ...
  parkalresult.crps <- data.frame(x=rep(100:130,5), 
                                  Strategy=factor(c(rep("ALM",31), rep("ALMC",31), rep("ALD",31), rep("Cokriging-CV",31), rep("MR-SUR",31)), 
...
for(j in 1:10){
  ...
  resultmatc[,j] <- c(r1, rmsematc[[j]][length(costmatc[[j]])])[1:31]
  if(ALC) resultmatc2[,j] <- c(r2, rmsematc2[[j]][length(costmatc2[[j]])])[1:31]
  resultmatc3[,j] <- c(r3, rmsematc3[[j]][length(costmatc3[[j]])])[1:31]
  resultmatc4[,j] <- c(r4, rmsematc4[[j]][length(costmatc4[[j]])])[1:31]
  resultmatco[,j] <- c(rco, rmsematco[[j]][length(costmatco[[j]])])[1:31]
  resultmatk[,j] <- c(rk, rmsematk[[j]][length(costmatk[[j]])])[1:31]
}
...
if(ALC){
  parkalresult <- data.frame(x=rep(100:130,6), 
                             Strategy=factor(c(rep("ALM",31), rep("ALC",31), rep("ALMC",31), rep("ALD",31), rep("Cokriging-CV",31), rep("MR-SUR",31)), 
                                             levels=c("ALD","ALM","ALC","ALMC","Cokriging-CV","MR-SUR")),
                             Mean = c(resultmeanc, resultmeanc2, resultmeanc3, resultmeanc4, resultmeanco, resultmeank),
                             Max = c(resultmaxc, resultmaxc2, resultmaxc3, resultmaxc4, resultmaxco, resultmaxk),
...
plotalpark <- ggarrange(plotalpark.rmse, NULL, plotalpark.crps, ncol=3, nrow=1, widths = c(1, -0.1, 1), common.legend = TRUE, legend="bottom")
plotalpark